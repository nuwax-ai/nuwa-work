...
**可提测**：双平台测试包已从上述提交构建，代码门禁全绿，macOS 与 Windows 均完成安装/启动态验收；登录态驱动的服务起停、未登录只起页面网关、旧产品目录零迁移、随包前端 + 企业登录入口均经真实进程验证。
...
7. **启动动画**：冷启动观察 loading 至少可见 800ms，初始化失败时应给出可重试提示而非白屏。
