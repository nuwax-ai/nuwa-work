...
7. 退让/收起轮（nuwax feat/ui-avoid-collapse@119067d2e 已合入 feat-dong.0930 + 壳 c8c989f3）：page-container 避让改平台分支——Win/Linux 恒避 TOP36（原 TOP+8=44 收窄），mac 展开不退让/整条侧栏收起才避 TOOLBAR44；page-container 收起态补 margin-left=@marginXs 与右下留白对称；壳 mac 顶行收窄为左侧 300px 图标/拖拽区（mac 不退让后全宽拖拽会挡内容顶部交互）、更新入口 mac 独立浮右上。
8. **收起语义修正（用户复审"这些任务没完成"后）**：原策略 `primarySidebarCollapsed: collapsed && !immersiveShell` 使沉浸壳下第 1 列（260px 单栏）恒不收起——点 ☰ 只有 200px 二级列消失，与"单栏+二级菜单一起收起/展开"的诉求不符。修正：策略去掉 `!immersiveShell` 短路（沉浸壳与浏览器同语义），并由 `SidebarNavLayout` 在沉浸壳下不渲染浮出展开钮（整栏开关由壳顶栏 ☰ 承担，浮出钮会重复并压内容）。nuwax feat-dong.0930 @ b4f5ed945。
9. **mac 环境的实证发现**：mac 壳 dev 加载 `localhost:3000`，由**另一个检出** `/Users/apple/workspace/nuwax`（独立 clone，非 nuwa-work 子模块）提供——壳侧改动与子模块改动都不会影响它；且该检出 `.umi` 生成缓存过期（残留已删的 AtMentionIcon 引用）导致应用白屏报 "Failed to compile"。处置：同步代码 + 清 `.umi` + 重启 umi dev（已在后台 nohup 运行）。后续 mac 验证须先确认该检出状态。验证结果：mac 收起整栏 + 8px 左留白 + 收起态 44px 退让 均实测通过；win-pc 已构建含修复 dist 并运行（收起点击待用户确认）。
