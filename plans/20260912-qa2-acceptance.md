...
> 后续 QA.4 真实重定向缺陷与修复见 [QA.4 验收](20260912-qa4-redirect-acceptance.md)；QA.3 已被取代为历史包。
...
2. ~~下一版候选包（`.3`）双平台构建与载荷级验收~~ **已完成**：mac 真机闭环 PASS（含退出 PID 消失）、win 安装+载荷对拍 PASS（退出验证受登录态阻塞，见「测试包」诚实边界节）。QA.2 的 Windows exe（SHA256 `75d4e3b02977c41eab8b909005ab99d01c52e3bbae55a15c6`）已产出但被 `.3` 取代，未安装，留档不验。
